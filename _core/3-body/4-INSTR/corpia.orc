...
iK	init 1-$ampvar			;dimensionless stiffness parameter.
iT30	init idur*4			;30db decay time in seconds.
ib 	limit .15-($ampvar*.15), 0, 1	;high-frequency loss parameter (keep this small).
...
iinit	init 7-($ampvar*4)		;the initial position of the hammer.
ipos	init .15+($ampvar*.05)			;position along the string that the strike occurs.
...
a1			repluck .15, 1, icps*1, .75, 1-$ampvar, apia
a2			repluck .5, 1, icps*2, .95, 1-$ampvar, apia
a3			repluck .95, 1, icps*3, .15, 1-$ampvar, apia
