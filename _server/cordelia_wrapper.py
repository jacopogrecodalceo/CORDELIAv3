				...
				if my_type == 'INSTR':
					for index, each_instr in enumerate(with_instr):
						if not each_instr in self.last_instr:
							self.out.append(separator + f'\tinstr {csound_instr_num + index}\n{each_instr}\n\tendin\n' + f'\tstart({csound_instr_num + index})\n')
							self.last_instr[index] = each_instr

					for index, each_instr in enumerate(self.last_instr):
						if each_instr:
							if not each_instr in with_instr:
								self.out.append(separator + f'\tkill({csound_instr_num + index})\n')
								self.last_instr[index] = None

				if my_type == 'ROUTE':
					csound_instr_num += len(self.last_instr)
					for index, each_instr in enumerate(with_instr):
